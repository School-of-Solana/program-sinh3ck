...
import { getMessageMetadataDecoder, MESSAGE_METADATA_DISCRIMINATOR } from '@project/anchor'
import { UiWalletAccount } from '@wallet-ui/react'

export function useGetMessagesMetadata({ account }: { account: UiWalletAccount }) {
  const query = useGetProgramAccountQuery()

  const filteredAccounts = query.data?.filter((account) => {
    const data = Buffer.from(account.account.data[0], 'base64')
    const discriminator = data.subarray(0, 8)
    return discriminator.equals(Buffer.from(MESSAGE_METADATA_DISCRIMINATOR))
  })

  const decoder = getMessageMetadataDecoder();
