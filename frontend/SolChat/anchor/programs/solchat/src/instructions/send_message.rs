...
#[derive(Accounts)]
#[instruction(_message_content: String, replytomessageid: Option<u32>)]
pub struct SendMessage<'info> {
    ...
    #[account(
        seeds = [MESSAGE_SEED.as_bytes(), parent.key().as_ref(), replytomessageid.clone().map_or_else(|| return Err(SolchatError::ReplyToMessageIDRequired), |msg_id| Ok(msg_id.to_be_bytes()))?.as_ref()],
        bump,
    )]
