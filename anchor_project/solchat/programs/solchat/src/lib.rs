...
pub mod solchat {
    ...
    pub fn send_message(ctx: Context<SendMessage>, message_content: String, reply_to_message_id: Option<u32>) -> Result<()> {
        msg!("Send message!");
        
        send_message_internal(ctx, message_content, reply_to_message_id)
    }

    pub fn react_to_message(ctx: Context<ReactToMessage>, emoji: String, message_id: u32) -> Result<()> {
        msg!("React to message!");
        
        react_to_message_internal(ctx, emoji, message_id)
    }
}
